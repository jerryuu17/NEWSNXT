          ...
          <nav className="hidden md:flex items-center space-x-6">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-sm font-medium text-foreground hover:text-red-accent transition-colors"
              >
                {item.name}
              </a>
            ))}
                ...
                <div className="flex flex-col space-y-4 mt-6">
                  {navItems.map((item) => (
                    <a
                      key={item.name}
                      href={item.href}
                      className="text-lg font-medium text-foreground hover:text-red-accent transition-colors"
                    >
                      {item.name}
                    </a>
                  ))}
        ...
        {isSearchOpen && (
          <div className="border-t py-3 animate-fade-in">
            <div className="relative max-w-md mx-auto">
              ...
              <Input
                placeholder="Search news, topics, or contributors..."
